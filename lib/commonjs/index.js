...
_config.default.addProcessorToMap(OP_GET_BY_KEY, 'OP_GET_BY_KEY', false, (left, right) => (0, _get2.default)(left, right));
...
function match(actual, expected, prefix) {
  ...
  for (let fieldName in expected) {
    ...
    let actualFieldValue = (0, _get2.default)(actual, fieldName);
...
function evaluateExpr(currentValue, expr, prefix, context, setOp) {
  ...
  if (typeExpr === 'string') {
    if (expr.startsWith('$$')) {
      ...
      return (0, _get2.default)(context[expr.substr(0, pos)], expr.substr(pos + 1));
    }
  ...
  for (let fieldName in expr) {
    ...
    let actualFieldValue = currentValue != null ? compleyKey ? (0, _get2.default)(currentValue, fieldName) : currentValue[fieldName] : undefined;
    ...
    if (typeof childFieldValue !== 'undefined') {
      ...
      if (compleyKey) {
        (0, _set2.default)(result, fieldName, childFieldValue);
      } else {
