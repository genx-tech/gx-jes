  ...
  addValidatorToMap: (tokens, tag, handler) => {
    tokens.forEach(op => {
      if (op in mapOfValidators) {
        throw new InvalidArgument(`Duplicate validator operator: ${op}`, {
          op,
          tag
        });
      }
    ...
    if (tag in validatorHandlers) {
      throw new InvalidArgument(`Duplicate handler for validation: ${tag}`);
    }
  ...
  addProcessorToMap: (tokens, tag, isUnary, handler) => {
    ...
    tokens.forEach(op => {
      if (op in mapOfProcessors) {
        throw new InvalidArgument(`Duplicate processor operator: ${op}`, {
          op,
          tag,
          unary: isUnary
        });
    ...
    if (tag in processorHandlers) {
      throw new InvalidArgument(`Duplicate handler for processing: ${tag}`);
    }
