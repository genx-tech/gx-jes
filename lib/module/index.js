...
const MSG = config.messages; //Validators
...
const OP_CAST_ARRAY = ['$castArray']; //Manipulate processors
...
const OP_IF = ['$if'];
config.addValidatorToMap(OP_EQUAL, 'OP_EQUAL', (left, right) => _isEqual(left, right));
config.addValidatorToMap(OP_NOT_EQUAL, 'OP_NOT_EQUAL', (left, right) => !_isEqual(left, right));
config.addValidatorToMap(OP_NOT, 'OP_NOT', (left, ...args) => !test(left, 'OP_MATCH', ...args));
config.addValidatorToMap(OP_GREATER_THAN, 'OP_GREATER_THAN', (left, right) => left > right);
config.addValidatorToMap(OP_GREATER_THAN_OR_EQUAL, 'OP_GREATER_THAN_OR_EQUAL', (left, right) => left >= right);
config.addValidatorToMap(OP_LESS_THAN, 'OP_LESS_THAN', (left, right) => left < right);
config.addValidatorToMap(OP_LESS_THAN_OR_EQUAL, 'OP_LESS_THAN_OR_EQUAL', (left, right) => left <= right);
...
config.addValidatorToMap(OP_MATCH, 'OP_MATCH', (left, right, prefix, context) => {
  if (Array.isArray(right)) {
...
config.addValidatorToMap(OP_MATCH_ANY, 'OP_MATCH_ANY', (left, right, prefix, context) => {
  if (!Array.isArray(right)) {
...
config.addProcessorToMap(OP_SUM, 'OP_SUM', true, left => _reduce(left, (sum, item) => {
  sum += item;
  return sum;
}, 0));
...
config.addProcessorToMap(OP_ENTRIES, 'OP_ENTRIES', true, left => _map(left, (value, key) => ({
  key,
  value
})));
config.addProcessorToMap(OP_CAST_ARRAY, 'OP_CAST_ARRAY', true, left => left == null ? null : Array.isArray(left) ? left : [left]);
config.addProcessorToMap(OP_GET_TYPE, 'OP_GET_TYPE', true, left => Array.isArray(left) ? 'array' : _isInteger(left) ? 'integer' : typeof left);
config.addProcessorToMap(OP_REVERSE, 'OP_REVERSE', true, left => _reverse(left));
config.addProcessorToMap(OP_ADD, 'OP_ADD', false, (left, right) => left + right);
config.addProcessorToMap(OP_SUB, 'OP_SUB', false, (left, right) => left - right);
config.addProcessorToMap(OP_MUL, 'OP_MUL', false, (left, right) => left * right);
config.addProcessorToMap(OP_DIV, 'OP_DIV', false, (left, right) => left / right);
config.addProcessorToMap(OP_SET, 'OP_SET', false, (left, right, prefix, context) => evaluateExpr(undefined, right, prefix, context, true));
config.addProcessorToMap(OP_ADD_ITEM, 'OP_ADD_ITEM', false, (left, right, prefix, context) => {
...
config.addProcessorToMap(OP_ASSIGN, 'OP_ASSIGN', false, (left, right, prefix, context) => {
  ...
  const rightValue = _mapValues(right, (expr, key) => evaluateExpr(left[key], expr, MSG.formatPrefix(key, prefix), { ...context,
    $$PARENT: left,
    $$CURRENT: left[key]
  }));
...
config.addProcessorToMap(OP_PICK, 'OP_PICK', false, (left, right, prefix) => {
  ...
  return _pickBy(left, (x, key) => match(key, right, MSG.formatPrefix(key, prefix))[0]);
});
config.addProcessorToMap(OP_GET_BY_INDEX, 'OP_GET_BY_INDEX', false, (left, right) => _nth(left, right));
config.addProcessorToMap(OP_GET_BY_KEY, 'OP_GET_BY_KEY', false, (left, right) => _get(left, right));
config.addProcessorToMap(OP_OMIT, 'OP_OMIT', false, (left, right, prefix) => {
  ...
  return _omitBy(left, (x, key) => match(key, right, MSG.formatPrefix(key, prefix))[0]);
});
config.addProcessorToMap(OP_GROUP, 'OP_GROUP', false, (left, right) => _groupBy(left, right));
config.addProcessorToMap(OP_SORT, 'OP_SORT', false, (left, right) => _sortBy(left, right));
...
config.addProcessorToMap(OP_MERGE, 'OP_MERGE', false, (left, right, prefix, context) => {
  if (!Array.isArray(right)) {
...
config.addProcessorToMap(OP_FILTER, 'OP_FILTER', false, (left, right, prefix
/*, context*/
) => {
  if (left == null) return null;
  ...
  return _filter(left, (value, key) => test(value, 'OP_MATCH', right, MSG.formatPrefix(key, prefix)));
});
config.addProcessorToMap(OP_REMAP, 'OP_REMAP', false, (left, right
/*, prefix, context*/
) => {
  if (left == null) return null;
...
config.addProcessorToMap(OP_IF, 'OP_IF', false, (left, right, prefix, context) => {
  if (!Array.isArray(right)) {
...
config.addProcessorToMap(OP_TO_JSON, 'OP_TO_JSON', true, left => left == null ? left : JSON.stringify(left));
config.addProcessorToMap(OP_TO_OBJ, 'OP_TO_OBJ', true, left => left == null ? left : JSON.parse(left));
...
function validateCollection(actual, collectionOp, op, expectedFieldValue, prefix) {
  ...
  switch (collectionOp) {
    case PFX_FOR_EACH:
      {
        const unmatchedKey = _findIndex(actual, item => !test(item, op, expectedFieldValue, prefix, context));
    ...
    case PFX_WITH_ANY:
      {
        const matched = _find(actual, item => test(item, op, expectedFieldValue, prefix, context));
...
function evaluateCollection(currentValue, collectionOp, opMeta, expectedFieldValue, prefix, context) {
  switch (collectionOp) {
    case PFX_FOR_EACH:
      return (Array.isArray(currentValue) ? _map : _mapValues)(currentValue, (item, i) => evaluateByOpMeta(item, expectedFieldValue, opMeta, MSG.formatPrefix(i, prefix), { ...context,
        $$PARENT: currentValue,
        $$CURRENT: item
      }));
...
function evaluateExpr(currentValue, expr, prefix, context, setOp) {
  ...
  if (setOp) {
    return _mapValues(expr, item => evaluateExpr(undefined, item, prefix, context, true));
  }
  ...
  for (let fieldName in expr) {
    ...
    let actualFieldValue = currentValue != null ? compleyKey ? _get(currentValue, fieldName) : currentValue[fieldName] : undefined;
