...
function transformCollection(currentValue, collectionOp, opMeta, expectedFieldValue, context) {
  ...
  switch (collectionOp) {
    case PFX_MAP:
      return (Array.isArray(currentValue) ? _map : _mapValues)(currentValue, (item, key) => applyOperator(item, expectedFieldValue, opMeta, getChildContext(context, currentValue, key, item)));
    ...
    case PFX_REDUCE:
      ...
      return _reduce(currentValue, (result, item, key) => applyOperator(result, expectedFieldValue[1], opMeta, getChildContext(context, currentValue, key, item)), expectedFieldValue[0]);
...
function transform(currentValue, expr, context, replaceLeft) {
  ...
  if (Array.isArray(expr)) {
    if (replaceLeft) {
      return expr.map(item => transform(undefined, item, { ...context
      }, true));
    }

    return expr.reduce((result, exprItem) => transform(result, exprItem, { ...context
    }), currentValue);
  }
  ...
  for (let fieldName in expr) {
    ...
    let complexKey = fieldName.indexOf('.') !== -1; //pick a field and then apply manipulation

    let actualFieldValue = currentValue != null ? complexKey ? _get(currentValue, fieldName) : currentValue[fieldName] : undefined;
    const childFieldValue = transform(actualFieldValue, expectedFieldValue, getChildContext(context, currentValue, fieldName, actualFieldValue));
