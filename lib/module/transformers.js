...
const UNARY = true;
const BINARY = false; //Query & aggregate operators (pure)
...
const OP_CAST_ARRAY = [ops.CAST_ARRAY, UNARY, '$castArray', '$makeArray']; //Math operators (pure)
...
const OP_MOD = [ops.MOD, BINARY, '$mod', '$remainder']; //Collection operators (pure)
...
const OP_TO_OBJ = [ops.TO_OBJ, UNARY, '$object', '$toObject', '$parseJSON']; //Value updater (pure)
...
const OP_INTERPOLATE = [ops.INTERPOLATE, BINARY, '$interpolate', '$template']; // [ <op name>, <unary> ]
...
config.addTransformerToMap(OP_MATCH, (left, right, context) => test(left, validators.MATCH, right, matchOptions, context));
config.addTransformerToMap(OP_SIZE, left => _size(left));
config.addTransformerToMap(OP_SUM, left => _reduce(left, (sum, item) => {
  sum += item;
  return sum;
}, 0));
config.addTransformerToMap(OP_GET_TYPE, left => Array.isArray(left) ? 'array' : Number.isInteger(left) ? 'integer' : typeof left);
...
config.addTransformerToMap(OP_OBJ_TO_ARRAY, left => _map(left, (v, k) => ({
  k,
  v
})));
config.addTransformerToMap(OP_PICK, (left, right, context) => {
  ...
  return _pickBy(left, (item, key) => test(key, validators.MATCH, right, matchOptions, getChildContext(context, left, key, item)));
});
config.addTransformerToMap(OP_OMIT, (left, right, context) => {
  ...
  return _omitBy(left, (item, key) => test(key, validators.MATCH, right, matchOptions, getChildContext(context, left, key, item)));
});
...
config.addTransformerToMap(OP_FILTER, (left, right, context) => {
  ...
  return _filter(left, (value, key) => test(value, validators.MATCH, right, matchOptions, {
...
config.addTransformerToMap(OP_ASSIGN, (left, right, context) => {
  ...
  const rightValue = _mapValues(right, (expr, key) => transform(left[key], expr, getChildContext(context, left, key, left[key])));
