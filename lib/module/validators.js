...
const MSG = config.messages; //Validators [ name, ...operator alias ]
...
config.addValidatorToMap(OP_NOT, function (left) {
  for (var _len = arguments.length, args = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
    args[_key - 1] = arguments[_key];
  }

  return !test(left, ops.MATCH, ...args);
});
...
config.addValidatorToMap(OP_LENGTH, (left, right, options, context) => test(_size(left), ops.MATCH, right, options, context));
...
config.addValidatorToMap(OP_MATCH, (left, right, options, context) => {
  if (Array.isArray(right)) {
    const errors = [];
    right.every(rule => {
      const reason = validate(left, rule, { ...options,
        asPredicate: false
      }, context);
...
config.addValidatorToMap(OP_ALL_MATCH, (left, right, options, context) => {
  ...
  const errors = [];
  left.every(leftItem => {
    const reason = validate(leftItem, right, { ...options,
      asPredicate: false
    }, context);
