...
var MSG = _config["default"].messages; //Validators [ name, ...operator alias ]
...
var OP_EXISTS = [_validateOperators["default"].EXISTS, '$exist', '$exists', '$notNull', '$required'];
var OP_MATCH = [_validateOperators["default"].MATCH, '$has', '$match', '$all'];
...
_config["default"].addValidatorToMap(OP_EQUAL, function (left, right) {
  return (0, _isEqual2["default"])(left, right);
});

_config["default"].addValidatorToMap(OP_NOT_EQUAL, function (left, right) {
  return !(0, _isEqual2["default"])(left, right);
});
...
_config["default"].addValidatorToMap(OP_GREATER_THAN, function (left, right) {
  return left > right;
});

_config["default"].addValidatorToMap(OP_GREATER_THAN_OR_EQUAL, function (left, right) {
  return left >= right;
});

_config["default"].addValidatorToMap(OP_LESS_THAN, function (left, right) {
  return left < right;
});

_config["default"].addValidatorToMap(OP_LESS_THAN_OR_EQUAL, function (left, right) {
  return left <= right;
});
...
_config["default"].addValidatorToMap(OP_NOT_IN, function (left, right) {
  if (right == null) {
    return true;
  }
...
_config["default"].addValidatorToMap(OP_START_WITH, function (left, right) {
...
_config["default"].addValidatorToMap(OP_END_WITH, function (left, right) {
  if (typeof left !== 'string') {
    return false;
  }
