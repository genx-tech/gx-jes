...
const formatName = (name, prefix) => {
    ...
    return fullName == null ? 'The value' : fullName;
};
...
export const messages = {
    ...
    //Exception messages
    SYNTAX_OP_NOT_ALONE:
        'Transformer operator can only be used alone in one pipeline stage.',
    SYNTAX_INVALID_EXPR: (expr) =>
        `Invalid expression syntax: ${JSON.stringify(expr)}`, // complext expr, not split out operator yet
    SYNTAX_INVALID_OP: (op, prefix) =>
        `Invalid operator "${op}" at ${formatPath(prefix)}.`,
    SYNTAX_NUMBER_AS_EXPR:
        'Number value cannot be used as a transformer expression.',

    INVALID_TRANSFORMER_OP: (op) => `Invalid transformer operator "${op}".`,
    UNSUPPORTED_VALIDATION_OP: (op, prefix) =>
        `Unsupported validation operator "${op}" at ${formatPath(prefix)}.`,
    INVALID_COLLECTION_OP: (op) => `Invalid collection operator "${op}".`,

    INVALID_TRANSFORMER_HANDLER: (tag) =>
        `Handler for transformer "${tag}" not found.`,
    INVALID_TEST_HANLDER: (tag) => `Handler for validator "${tag}" not found.`,

    INVALID_OP_EXPR: (op, right) =>
        `Invalid "${op}" expression: ${JSON.stringify(right)}.`,
    INVALID_COLLECTION_OP_EXPR: (collectionOp, op, right) =>
        `Invalid "${op}" expression for collection "${collectionOp}" traversing: ${JSON.stringify(
            right
        )}.`,
    ...
    REQUIRE_RIGHT_OPERAND: (op) =>
        `Binary operator "${op}" requires a right operand.`,
    RIGHT_OPERAND_NOT_EMPTY: (op) =>
        `Unary operator "${op}" does not require a right operand.`,
...
const config = {
    ...
    addValidatorToMap: (tokens, handler) => {
        ...
        alias.forEach((op) => {
            if (op in mapOfValidators) {
                throw new Error(
                    `Duplicate validator alias "${op}" for operator "${tag}".`
                );
            }
    ...
    addTransformerToMap: (tokens, handler) => {
        ...
        alias.forEach((op) => {
            if (op in mapOfTransformers) {
                throw new Error(
                    `Duplicate transformer alias: "${op}" for operator "${tag}".`
                );
            }
    ...
    loadMessages: (moreMessages) =>
        Object.assign(config.messages, moreMessages),
};
